...
const getLivrabilById = async (req, res) => {
  try {
    ...
    if (!livrabil)
      return res.status(404).json({ message: "Livrabilul nu a fost găsit" });
    res.json(livrabil);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
const createLivrabil = async (req, res) => {
  try {
    const { titlu, descriere, link, data, idProiect, idJuriu } = req.body;
...
const getAllLivrabileByIdProiect = async (req, res) => {
  try {
